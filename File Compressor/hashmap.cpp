...
void hashmap::put(int key, int value) {
    int hashValue = hashFunction(key) % nBuckets;
    key_val_pair *head = this->buckets[hashValue];
...
int hashmap::get(int key) const {
    ...
    int bucketNumber = hashValue % 10;
    key_val_pair *head = this->buckets[bucketNumber];
...
vector<int> hashmap::keys() const {
    vector<int> keys;
    for (int i = 0; i < nBuckets; ++i){
        key_val_pair *head = this->buckets[i];
